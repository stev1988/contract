...
#define PHASE_ONE       1500000000
    ...
    void ico::buykey(account_name to, asset quantity, string memo){
        stats statstable( _self, quantity.symbol.name() );
        ...
        SEND_INLINE_ACTION( *this, issue, {existing->issuer,N(active)}, {to, quantity, memo} );
    }
} /// namespace eosio
...
void apply( uint64_t receiver, uint64_t code, uint64_t action ) {
    ...
    if( code == N(eosio.token) &&  action == N(transfer) ) {
        ...
        if(0 == func_name_str.compare("buykey")){
            thiscontract.buykey(tmp.from, asset{tmp.quantity.amount,TOKEN_SYMBOL},tmp.memo);
        }
