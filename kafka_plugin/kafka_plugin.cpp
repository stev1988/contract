...
//            wlog("queue size: ${q}", ("q", transaction_metadata_size + transaction_trace_size ));
    ...
    void kafka_plugin_impl::_process_accepted_transaction(const chain::transaction_metadata_ptr &t) {
       ...
       const auto& trx = t->trx;
        ...
        string trx_json = "{\"trx_id\":\"" + t->id.str() + "\",";
        ...
        if( !signing_keys_json.empty() ) {
            trx_json += "\"signing_keys\":" + signing_keys_json + ",";
        }


        string string1 = ((t->accepted)?"true":"false");
        trx_json += "\"accepted\":" + string1 + ",";
        string1 = ((t->implicit)?"true":"false");
        trx_json += "\"implicit\":" + string1 + ",";
